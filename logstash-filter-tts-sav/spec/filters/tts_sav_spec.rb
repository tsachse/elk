...
describe LogStash::Filters::TtsSav do
  describe "Telegrammbeschreibung einlesen" do
    ...
    sample("BOFWT01TRKR08.03.1605:39:020442") do
      ...
      insist { subject["@timestamp"] } == LogStash::Timestamp.new(tz.local_to_utc(Time.strptime("08/03/16 05:39:02", '%d/%m/%y %H:%M:%S')))
    end
  end
  ...
  describe "Telegrammblock vearbeiten" do
    ...
    sample(["BOFWT01TRKR08.03.1605:39:020452","TG10675014002","EOF04520003"]) do
      ...
      tz = TZInfo::Timezone.get('Europe/Berlin')
      insist { subject[0]["@timestamp"] } == LogStash::Timestamp.new(tz.local_to_utc(Time.strptime("08/03/16 05:39:02", '%d/%m/%y %H:%M:%S')))
      ...
      insist { subject[1]["@timestamp"] } == LogStash::Timestamp.new(tz.local_to_utc(Time.strptime("08/03/16 05:39:02", '%d/%m/%y %H:%M:%S')))
    end
  end
